...
class Go2Env:
    ...
    # ------------ reward functions----------------
    # 로봇의 보행 안정성은 어느 정도 확보가 된 상황이므로 뺄건 빼자
    ...
    def _reward_tracking_ang_vel(self):
        ...
        #로봇이 요구된 각속도(commands[:, 2])를 정확히 따라가도록 보상.
        #오차가 작을수록 높은 보상을 부여.
        return torch.exp(-ang_vel_error / self.reward_cfg["tracking_sigma"])

    def _reward_lin_vel_z(self):
        # Penalize z axis base linear velocity
        # 로봇이 점프하거나 위아래로 튀는 행동을 방지.
        # Z축 속도가 크면 페널티를 부여.
        return torch.square(self.base_lin_vel[:, 2])

    def _reward_action_rate(self):
        # Penalize changes in actions
        #로봇이 너무 급격한 행동(action change)을 하지 않도록 유도.
        return torch.sum(torch.square(self.last_actions - self.actions), dim=1)

    def _reward_similar_to_default(self):
        # Penalize joint poses far away from default pose
        #로봇의 관절이 기본자세(default pose)를 크게 벗어나지 않도록 유도.
        return torch.sum(torch.abs(self.dof_pos - self.default_dof_pos), dim=1)

    def _reward_base_height(self):
        # Penalize base height away from target
        # 로봇이 너무 낮거나 높아지면 보상을 감소.
        # 이를 통해 안정적인 보행을 유지하도록 학습.
